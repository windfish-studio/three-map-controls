    ...
    THREE.MapControls = function ( object, domElement, options ) {
        ...
        this.update = function() {
            ...
            return function update () {
                ...
                var za = updateZoomAlpha();
                position.lerpVectors(minZoomPosition, maxZoomPosition, za);
        ...
        function updateZoomAlpha(){
            ...
            currentZoom -= diff * damping_alpha;
            var rounding_places = 100000000;
            zoomAlpha = Math.abs(Math.round((1 - (currentZoom / (scope.maxDistance - scope.minDistance))) * rounding_places ) / rounding_places);
        ...
        function calculateMinZoom(cam_pos, maxzoom_pos){
            return maxzoom_pos.clone().add(
                cam_pos.clone()
                        .sub(maxzoom_pos)
                        .normalize()
                        .multiplyScalar(scope.maxDistance - scope.minDistance)
                );
        }
...
})(window.THREE || require('three'), window._ || require('lodash'));